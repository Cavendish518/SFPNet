...
def voxelize(coord, voxel_size=0.05, hash_type='fnv', mode=0):
    ...
    # _, inv_idx, count = np.unique(key_sort, return_counts=True, return_inverse=True)
    if mode == 0:  # train mode
        idx_select = np.cumsum(np.insert(count, 0, 0)[0:-1]) + np.random.randint(0, count.max(), count.size) % count
        idx_unique = idx_sort[idx_select]

        # new projback
        sorted_idx = np.zeros(key.shape[0]).astype(np.int64)
        # sorted_idx[idx_unique] = 1
        sorted_idx[idx_select] = 1
        sorted_idx = np.cumsum(sorted_idx) - 1
        idx_recon = np.zeros(key.shape[0]).astype(np.int64)
